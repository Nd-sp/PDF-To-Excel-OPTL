/**
 * Ascend Telecom Infrastructure Excel Format Template
 * Based on Ascend Telecom Infrastructure Zoho Format.xlsx
 * Total: 109 columns
 */
...
module.exports = {
  ...
  // Field extraction patterns for Ascend PDFs
  // NOTE: pdf-parse removes spaces, so use \s* (zero or more) instead of \s+ (one or more)
  // NOTE: In Ascend PDFs, labels and values are on separate lines with vendor name in between
  extractionPatterns: {
    vendor_name: /Ascend\s+Telecom\s+Infrastructure\s+Pvt\.?\s*Ltd?\.?/i,
    bill_number: /Invoice\s*No\s*:\s*[\s\S]{0,150}?([A-Z]{2,4}\/\d{6}\/\d{4})/i,
    bill_date: /Invoice\s*Date\s*:\s*[\s\S]{0,150}?(\d{2}\/\d{2}\/\d{4})/i,
    due_date: /Due\s*Date\s*:\s*[\s\S]{0,150}?(\d{2}\/\d{2}\/\d{4})/i,
    subtotal: /Taxable\s*(?:Value|Amount)\s+(?:Less)?[\s\S]{0,50}?([0-9]{3,}(?:\.\d{2})?)/i,
    total: /Grand\s*Total\s+([0-9,]+\.?\d{2})/i,
    cgst: /CGST\s+(?:\d+\.?\d+\s*)?\n\s*([0-9,]+\.?\d{2})/i,
    sgst: /SGST(?:\/UTGST)?\s+(?:\d+\.?\d+\s*)?\n\s*([0-9,]+\.?\d{2})/i,
    igst: /IGST\s+(?:\d+\.?\d+\s*)?\n\s*([0-9,]+\.?\d{2})/i,
    cgst_rate: /CGST\s*Rate\s*of\s*Tax\s*%[^\d]*?(\d+\.?\d+)/i,
    sgst_rate: /SGST(?:\/UTGST)?\s*Rate\s*of\s*Tax\s*%[^\d]*?(\d+\.?\d+)/i,
    igst_rate: /IGST\s*Rate\s*of\s*Tax\s*%[^\d]*?(\d+\.?\d+)/i,
    gstin: /GSTIN\s*:\s*[\s\S]{0,150}?([0-9]{2}[A-Z]{5}[0-9]{4}[A-Z][0-9A-Z]{3})/i,
    customer_gstin: /GST\s*TIN\s*:\s*[\s\S]{0,100}?([0-9]{2}[A-Z]{5}[0-9]{4}[A-Z][0-9A-Z]{3})/i,
    place_of_supply: /Place\s*of\s*Supply\s*:\s*[\s\S]{0,100}?(\d{2})-?([A-Za-z]+)/i,
    hsn_sac: /HSN\s*\/\s*SAC\s*[\s\S]{0,100}?(\d{6})/i,
    pan: /Pan\s*No\s*:\s*[\s\S]{0,150}?([A-Z]{5}\d{4}[A-Z])/i,
    description: /Description\s*of\s*Service\s*[\s\S]{0,100}?([A-Za-z\s]+?)(?:\d|Amount)/i,
    bill_period_from: /Bill\s*Period\s*From\s*[\s\S]{0,100}?(\d{2}\/\d{2}\/\d{4})/i,
    bill_period_to: /Bill\s*Period\s*To\s*[\s\S]{0,100}?(\d{2}\/\d{2}\/\d{4})/i,
    irn: /IRN\s*:\s*[\s\S]{0,100}?([a-f0-9]{64})/i,
    account_number: /Account\s*No\s*:\s*[\s\S]{0,100}?(\d+)/i,
    ifsc: /IFSC\s*Code\s*:\s*[\s\S]{0,100}?([A-Z]{4}\d{7})/i,
    tax_amount: /Tax\s*Amount\s*[\s\S]{0,100}?([0-9,]+\.?\d{2})/i
  },
  ...
  defaults: {
    ...
    vendor_name: 'Ascend Telecom Infrastructure Private Ltd',
    usage_unit: 'Month',
    quantity: 1,
    item_name: 'CO-LO CHARGES / TOWER INFRA ON LEASE - EXP',
    account: 'ASCEND CO-LO EXP'
  }
};
